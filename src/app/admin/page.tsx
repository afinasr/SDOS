    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <Link href={stat.href} key={i} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-3xl p-3.5 flex flex-col justify-between h-24 transition-colors hover:border-cyan-500 dark:hover:border-cyan-500 hover:shadow-lg hover:shadow-cyan-500/10 block cursor-pointer">
      ...
      <div className="space-y-3 pt-2">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-3xl overflow-hidden divide-y divide-zinc-100 dark:divide-white/5 transition-colors">
          {[
            ...
            <Link key={i} href={`/admin/projects/${shoot.names.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="p-3 sm:p-4 flex items-center justify-between hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors cursor-pointer block">
              <div className="flex items-center gap-3">
                <div className="bg-zinc-50 dark:bg-white/10 border border-zinc-200 dark:border-white/10 rounded-xl w-10 h-10 flex flex-col items-center justify-center">
                  <span className="text-base font-bold text-cyan-600 dark:text-cyan-400 leading-none">{shoot.day}</span>
                  <span className="text-[9px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase mt-0.5">{shoot.month}</span>
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-zinc-900 dark:text-white">{shoot.names}</h4>
                  <p className="text-[11px] text-zinc-500 mt-0.5">{shoot.venue}</p>
                </div>
              </div>
        ...
        <div className="space-y-2.5">
          {[
            ...
            <Link key={i} href={`/admin/projects/${proj.names.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="block bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-3xl p-3.5 relative overflow-hidden transition-colors hover:bg-zinc-50 dark:hover:bg-white/5 cursor-pointer">
              ...
              <h4 className="font-bold text-sm text-zinc-900 dark:text-white pr-16">{proj.names}</h4>
              <p className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-0.5">{proj.type}</p>
