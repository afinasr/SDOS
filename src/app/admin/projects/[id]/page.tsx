...
export default function ProjectDetailsView({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [lineItems, setLineItems] = useState([
    { id: 1, desc: "Wedding Coverage (2 Days)", price: 80000 },
    { id: 2, desc: "Cinematic Film", price: 40000 }
  ]);
  ...
  const toggleMilestone = (idx: number) => {
    ...
    updated[idx].status = updated[idx].status === "Paid" ? "Pending" : "Paid";
    setMilestones(updated);
  };

  const toggleCrew = (idx: number) => {
    const updated = [...crew];
    updated[idx].assigned = !updated[idx].assigned;
    setCrew(updated);
  };
